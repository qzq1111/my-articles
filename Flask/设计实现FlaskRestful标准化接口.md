...
`{"code":0,"data":{"age":18,"name":"zhang"},"msg":"成功"}`
## 4.实现全球化配置
实现思路是编写两份不一样的响应消息，前端通过选择语言，后端返回对应语言的消息。因此采用yaml编写响应消息，并加载到Flask应用中，通过语言选择返回不同的语言消息。在我的另外一篇文章[《Flask加载yaml配置文件》](https://blog.csdn.net/qq_22034353/article/details/88591681)中介绍了Flask加载配置yaml文件。
